...
import { writeComment, getComments, saveBlogPosts, getBlogPostsList } from '../../lib/services-aux.js'
import { cloudinaryStorageBlogCover } from "./blogPosts-utils.js";
import { createPDFReadableStream } from './pdf-utils.js'
import { pipeline } from "stream";
import {sendBlogConfirmationEmail} from '../../lib/email/email-aux.js'
...
blogPostRouter.post("/", blogPostValidationFields, async (req, res, next) => {
    try {
...
// POST IMG
blogPostRouter.post("/:id/uploadCover", multer({ storage: cloudinaryStorageBlogCover }).single("blogPostImg"), async (req, res, next) => {
    try {
        const errorList = validationResult(req)
        if (!errorList.isEmpty()) {
            next(createHttpError(400, { errorList }))
        ...
        if (!currentBlogPost) {
            ...
            await sendBlogConfirmationEmail(blogPostAddedImg)
...
blogPostRouter.get("/download/:id", async (req, res, next) => {
    try {
        const blogPosts = getBlogPosts()
        const selectedBlogPost = blogPosts.find(blogPost => blogPost._id === req.params.id)
        ...
        // set header
        res.setHeader("Content-Disposition", `attachment; filename=${selectedBlogPost.title}.pdf`)
        // set destination
