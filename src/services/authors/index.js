...
import {cloudinaryStorageBlogAuthorAvatar} from '../blogPosts/blogPosts-utils.js'
...
authorsRouter.get("/", (req, res) => {
    try {
        ...
        res.send(authorsList).status(200)
    } catch (error) {
...
authorsRouter.post("/", (req, res) => {
    try {
        ...
        if (checkAuthor(file, req.body.email)) {
            ...
            res.status(403).send({msg: 'E-mail already exists'})
        }
...
authorsRouter.post("/:id/uploadAvatar", multer({storage: cloudinaryStorageBlogAuthorAvatar}).single('avatarImg'), validateImgExtension, authorHandlers.addAvatar)
